...
null1.pval.t1 <- p.expt1
null1.pval.t2 <- p.expt2
sig1.stat.t1 <- stat1
sig1.stat.t2 <- stat2
sig1.pval.t1 <- p.expt1
sig1.pval.t2 <- p.expt2
null2.pval.t1 <- p.expt1
null2.pval.t2 <- p.expt2
sig2.pval.t1 <- p.expt1
sig2.pval.t2 <- p.expt2
null3.pval.t1 <- p.expt1
null3.pval.t2 <- p.expt2
sig3.pval.t1 <- p.expt1
sig3.pval.t2 <- p.expt2

sigalt.pval.t1 <- p.expt1
sigalt2.pval.t1 <- p.expt1

sigalt.5.stat.t1 <- stat1
sigalt.5.stat.t2 <- stat2

for (i in 1:11) {
  print(mean(null3.pval.t2[[i]]<.03|null3.pval.t1<.03))
}
...
for (i in 1:nsim) {
  ev.c[i] <- sum(ifelse(ctrl.list[[i]][1:n.fin]>t.length, 0, 1))
  ev.t[i] <- sum(ifelse(tx1.list[[i]]>t.length,0,1))
}